...
export const useUserTasksAPI = () => {
  ...
  const statusFilter = `FIND("Not Applicable", {Status}) = 0, FIND("Cancelled", {Status}) = 0, FIND("Terminated", {member_status}) = 0, {is_test_member (from Member)} = 0, OR(FIND("Complete", {Status}) = 0, AND(FIND("Complete", {Status}) != 0, IS_SAME({last_modified}, TODAY(), 'day'))),`
