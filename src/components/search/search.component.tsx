...
const SearchInput = () => {
  ...
  const searchMembers = (q: string)  => {
      const throttleFunc = throttle(1000, true, () => {
          if(q) {
            setIsLoading(true);
            ...
            airtableFetch(`members/list/0?${searchQ}`).then((response: any) => {
                ...
                if(response && Object.keys(response).length > 0) {
                  finalRes = Object.keys(response).map((recId: string) => ({
                    ...response[recId],
                    id: recId,
                    displayName: [response[recId]['Full Name'], `${response[recId]['Age']}Yrs`,response[recId]['Sex'], response[recId]['Employer']].join(', ')
                  }));
                  setResults(finalRes)
                }
                setResults(finalRes)
  ...
  const onItemChange = (item: resultItemType | null, stateAndHelpers: {clearSelection : Function}) => {
    item && history.push(`/member/${item.id}`, { from: history.location });
    // clear selection
    ...
    >
      {({
        ...
        <div className={styles.searchWrap}>
          <div className={styles.searchInputWrap} {...getRootProps({}, { suppressRefError: true })}>
            <SearchIcon/>
            <input type="text" {...getInputProps()} className={styles.search} placeholder="Search members..." role="searchbox"/>
            {inputValue &&
              <button className="btn-icon" onClick={() => clearSelection()} aria-label="toggle menu" data-testid="search-input-clear">
                <CloseIcon/>
              </button>
            }
          </div>
            ...
            <div className={styles.searchResultsWrap}>
                    ...
                    results
                      .map((item: any, index: number) => (
                        <li {...getItemProps({
                              ...
                              style: {
                                backgroundColor:
                                  highlightedIndex === index ? 'var(--orange-light)' : 'var(--white)',
              ...
              }
              {
                ...
                <div className={styles.noResultWrap}>
                  <span className={styles.noResultMsg}>Your search - <strong>{inputValue}</strong> - did not match any member. </span>
